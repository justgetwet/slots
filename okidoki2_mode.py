...
def set_mode(s, role):
  ...
  win = np.empty(9, dtype=np.object_)
  win[0:4] = "No rare"
  win[4] = "cherry"
  win[5] = "suika"
  win[6] = "reach"
...
def change_mode(s, role, mode):
  ...
  if win[role] in ["No rare", "cherry"] and mode == 0: # mode A
  ...
  if win[role] in ["cherry", "suika"] and mode == 8: # ensure
    if next_mode == 6:
      next_mode += 2
  
  if win[role] in ["reach", "kcherry"] and mode == 8: # ensure
    if next_mode == 7:
      next_mode += 1
